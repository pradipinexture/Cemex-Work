	...
	try {
		...
		if (responseCode >= 200 && responseCode < 300) {
			...
			if (duplicates && duplicates.size() > 0) {
				...
				def existingLocation = getLocation(duplicateId);
				if(existingLocation != null) {
					locationData.put("businessId", existingLocation?.response?.location?.businessId)
				}
...
def getLocation(def locationId) {
	try {
		...
		def url = new URL("${UBERALL_ENDPOINT}/${locationId}")
		def connection = url.openConnection()
...
if (className.equals("com.liferay.journal.model.JournalArticle")) {
	try {
		...
		//Update Friendly URL
		def contextPath = "locations/"
		def companyId = Long.parseLong(workflowContext.get("companyId"))
		try {
			if (!article.getUrlTitle().contains(contextPath)) {
				def classNameId = ClassNameLocalServiceUtil.getClassNameId(JournalArticle.class.getName());
				def friendlyURLEntries = FriendlyURLEntryLocalServiceUtil.getFriendlyURLEntries(groupId, classNameId, article.getResourcePrimKey())
		
				if (!friendlyURLEntries.isEmpty()) {
					def existingEntry = friendlyURLEntries[0]
					def urlTitleMap = new HashMap<String, String>()

					def defaultLanguageId = LocaleUtil.toLanguageId(LocaleUtil.getSiteDefault())
		
					urlTitleMap.put(defaultLanguageId, contextPath+ article.getUrlTitle())
					urlTitleMap.put("en_GB", contextPath+ article.getUrlTitle())
		
					def updatedEntry = FriendlyURLEntryLocalServiceUtil.updateFriendlyURLEntry(existingEntry.getFriendlyURLEntryId(), classNameId, article.getResourcePrimKey(), defaultLanguageId, urlTitleMap)
			
				}
			}
		} catch (Exception e) {
			println("Error updating article friendly URL: ${e.getMessage()}")
		}
