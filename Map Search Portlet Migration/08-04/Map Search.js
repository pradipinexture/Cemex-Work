...
async function loadLocations() {
  try {
    ...
    const ukFilter = `contentFields/Country eq '${currentCountry}'`;
    filterString += filterString ? ` and ${ukFilter}` : `&filter=${ukFilter}`;

    // Make API request
    const response = await fetch(`${baseUrl}?fields=${neccesaryfieldsFromAPI}&pageSize=${pageSize}${currentSearchTerm ? `&search="${currentSearchTerm}"` : ""}${filterString}`, {
      method: 'GET',
      headers: {
