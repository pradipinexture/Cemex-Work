...
document.getElementById('save-file').addEventListener('click', async function (e) {
    ...
    if (operationType === "Save") {
      const result = await uploadFileToLiferay('file', documentMediaFolderId, null, true);
      if(!result) {
         apiErrorStep();
         return;
      }
        ...
        var fileDiv = generateFileCard(fileName, result);
...
function apiErrorStep() {
   Toast.success(fileNotUploadedRenameIt);
