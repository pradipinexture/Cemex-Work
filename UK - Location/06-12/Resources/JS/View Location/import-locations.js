     ...
     try {
        ...
        for(var csvLine = 1; csvLine < lines.length; csvLine++) {
            ...
            if(generatedArticleResource.locationId && generatedArticleResource.locationId != "-") {
                await updateArticle(null,generatedArticleResource.xmlData, generatedArticleResource.locationId, generatedArticleResource.name, generatedArticleResource.articleProductCategories, true);
            } else {
...
async function processCSVData(line, headers) {
    ...
    createDynamicElement(locationFieldsIds.isCSVLocation.xmlFieldName, "isCSVLocation", locationFieldsIds.isCSVLocation.type, true, true)
    ...
    await createDynamicElement(locationFieldsIds["Geolocation"].xmlFieldName, 
        locationFieldsIds["Geolocation"].fieldName, 
        locationFieldsIds["Geolocation"].type, 
    ...
    async function processOpeningHours(dayName, value) {
        ...
        var openingDynamicElement = await getDynamicElement(
            config.xmlFieldName, 
            config.fieldName, 
            config.type, 
            ''
        ...
        var dayNameElement = await createDynamicElement(
            openingNested.dayName.xmlFieldName, 
            openingNested.dayName.fieldName, 
            openingNested.dayName.type, 
            dayName, 
            false
        );
        ...
        var isClosedElement = await createDynamicElement(
            openingNested.isClosed.xmlFieldName, 
            openingNested.isClosed.fieldName, 
            openingNested.isClosed.type, 
            isClosed, 
            false
        );
    ...
    async function generateOpenCloseElement(config, openTime="", closeTime="") {
        ...
        var opClsParentElement = await getDynamicElement(
            opClsParentConfig.xmlFieldName, 
            opClsParentConfig.fieldName, 
            opClsParentConfig.type, 
            ''
        );

        var openElement = await createDynamicElement(
            opClsConfig.open.xmlFieldName, 
            opClsConfig.open.fieldName, 
            opClsConfig.open.type, 
            openTime, 
            false
        );

        var closeElement = await createDynamicElement(
            opClsConfig.close.xmlFieldName, 
            opClsConfig.close.fieldName, 
            opClsConfig.close.type, 
            closeTime, 
            false
        );
