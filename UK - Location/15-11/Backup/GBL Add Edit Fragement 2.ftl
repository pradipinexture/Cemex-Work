   ...
   document.addEventListener('DOMContentLoaded', function () {
      ...
      function findFieldConfigByFieldRef(fieldRef) {
       ...
       async function processOpeningHours(dayElement, fieldConfig) {
            ...
            const startTime = dayElement.querySelector(`[field-reference="Open"] dynamic-content`)?.textContent;
            const closeTime = dayElement.querySelector(`[field-reference="Close"] dynamic-content`)?.textContent;
            const isClosed = dayElement.querySelector(`[field-reference="isClosed"] dynamic-content`)?.textContent === 'true';

            if (isClosed) timeValue = "Closed";
            else if (startTime && closeTime) timeValue = startTime + ' - ' + closeTime;
