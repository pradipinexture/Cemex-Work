         ...
         if (nestedElements.length > 0) {
            ...
            if (name.includes("OpeningHours")) {
               ...
               if(dynamicElement.children.length == 4) {
                  ...
                  let dayObject = {};
                  var isClosed = extractvalueByFieldRef(dynamicElement, "isClosed");
                  var dayName = extractvalueByFieldRef(dynamicElement, "DayName") || "Day";

                  if(isClosed == "true") {
                     dayObject['isClosed'] = true;
                  } else {
                     var openTime = extractvalueByFieldRef(dynamicElement, "Open");
                     var closeTime = extractvalueByFieldRef(dynamicElement, "Close");
                     if(openTime && closeTime) {
                        dayObject['openTime'] = openTime;  
                        dayObject['closeTime'] = closeTime; 
                     }
                  }
                  elementMap.get(dayName).push(dayObject);
               }
            } else {
...
function extractvalueByFieldRef(dynamicElement, fieldName) {
