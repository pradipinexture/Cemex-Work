...
async function convertObjectTOCSVData(locationData) {
    ...
    const rows = await Promise.all(locationData.map(async (location) => {
        const fieldValues = await Promise.all(locationStructureFields.map(async (field) => {
            ...
            if (Array.isArray(fieldValue) && fieldValue.length == 1) {
                if (field.endsWith("day")) {
                    if (fieldValue[0].isClosed == "true") {
                        ...
                        return Object.entries(fieldValue[0])
                            .map(function(entry) {
                    ...
                    return fieldValue.map((item, i) => Object.keys(item).map(key => key + '_' + (i+1) + ' : ' + item[key]).join(',  ')).join('\n') + ","
                }
            } 
            return fieldValue || "-";
        }));
