        ...
        for (var dynamicElement of dynamicElements) {
            ...
            if (nestedElements.length > 0) {
                if (name == "OpeningHours") {
                    var dayObj = {};
                    var dayValue = getElementValueByName(dynamicElement, "DayName");
                    ...
                    if (getElementValueByName(dynamicElement, "isClosed") == "true") {
                        ...
                        for (var op of nestedElements) {
                            ...
                            var dayOpenTime = getElementValueByName(op, "Open");
                            var dayCloseTime = getElementValueByName(op, "Close");
...
function xmlJsonToString(jsonString) {
    // First parse the JSON string to handle it as an object
    const jsonObj = JSON.parse(jsonString);
    
    // Convert back to string with proper escaping
    let csvSafeJson = JSON.stringify(jsonObj)
        // Escape quotes by adding a backslash
        .replace(/"/g, '\\"')
